...
let player3ws: WebSocket;
let player4ws: WebSocket;
...
beforeAll(async () => {
    ...
    player1ws = await connectPlayer(roomCode, TEST_PORT, (data: any) => {
        if (data.type === "player_board") {globalThreat = data.payload.threat;}
        if(data.type === "game_state") {if(data.payload.state === "end_state") win = data.payload.win}
    });
    ...
    player3ws = await connectPlayer(roomCode, TEST_PORT, (data: any) => {
        if (data.type === "player_board") {globalThreat = data.payload.threat;}
        if(data.type === "game_state") {if(data.payload.state === "end_state") win = data.payload.win}
    });

    player4ws = await connectPlayer(roomCode, TEST_PORT, (data: any) => {
        if (data.type === "player_board") {globalThreat = data.payload.threat;}
        if(data.type === "game_state") {if(data.payload.state === "end_state") win = data.payload.win}
    });
    ...
    player3ws.send(JSON.stringify({ type: "room", payload: { ready: true } }));
    player4ws.send(JSON.stringify({ type: "room", payload: { ready: true } }));
...
describe("Game flow tests", () => {
    ...
    test("Verify game is losable with timeout", async () => {
        await wait(CONFIG.DEFAULT_GAME_DURATION + 2000);
